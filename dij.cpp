...
char get_min_distance_node(const unordered_map<char, int>& distances, const set<char>& visited) {
...
pair<int, vector<char>> dijkstra(char start_node, char end_node) {
    ...
    if (total_cost == numeric_limits<int>::max()) {
        ...
        return {total_cost, {}}; // Return empty path for unreachable nodes
    }

    // Reconstruct path
    for (char at = end_node; at != start_node; at = previous_nodes[at]) {
        if (previous_nodes.find(at) == previous_nodes.end()) {
            cout << "No path found from " << start_node << " to " << end_node << ".\n";
            return {numeric_limits<int>::max(), {}}; // Handle incorrect path
        }
        path.push_back(at);
    }
...
int main() {
    ...
    if (graph.find(start_node) == graph.end() || graph.find(end_node) == graph.end()) {
        cout << "Error: One or both of the nodes are not present in the graph.\n";
        return 1;
    }
